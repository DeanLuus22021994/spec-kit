...
main() {
    ...
    if [[ ! -f "$TASKS" ]]; then
        logger_error "tasks.md not found in $FEATURE_DIR"
        logger_info "Run /tasks first."
        return 1
    fi
    ...
    if $json_mode; then
        ...
        logger_info "Checking available docs:"
        ...
        check_file_exists "$TASKS" "tasks.md"
    fi
}

invoke_speckit_block "Check-Impl-Prereqs" main "$@"
