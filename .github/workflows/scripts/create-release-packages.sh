...
generate_commands() {
  ...
  for template in .specify/templates/commands/*.md; do
    ...
    description=$(awk -v cmd="$name" '
      $0 ~ "^"cmd":" { in_cmd=1; next }
      in_cmd && /^  description:/ { sub(/^  description:[[:space:]]*/, ""); print; exit }
      in_cmd && /^[a-zA-Z]/ { exit }
    ' "$config_file")

    # Extract script command from config file
    script_command=$(awk -v cmd="$name" -v sv="$script_variant" '
      $0 ~ "^"cmd":" { in_cmd=1; next }
      in_cmd && /^  scripts:/ { in_scripts=1; next }
      in_scripts && /^[[:space:]]*'"$script_variant"':/ { sub(/^[[:space:]]*'"$script_variant"':[[:space:]]*/, ""); print; exit }
      in_scripts && (/^  [a-zA-Z]/ || /^[a-zA-Z]/) { exit }
    ' "$config_file")

    if [[ -z $script_command ]]; then
      echo "Warning: no script command found for $script_variant in $template (config: $name)" >&2
      script_command="(Missing script command for $script_variant)"
    fi

    # Extract agent_script command from config file
    agent_script_command=$(awk -v cmd="$name" -v sv="$script_variant" '
      $0 ~ "^"cmd":" { in_cmd=1; next }
      in_cmd && /^  agent_scripts:/ { in_agent_scripts=1; next }
      in_agent_scripts && /^[[:space:]]*'"$script_variant"':/ { sub(/^[[:space:]]*'"$script_variant"':[[:space:]]*/, ""); print; exit }
      in_agent_scripts && (/^  [a-zA-Z]/ || /^[a-zA-Z]/) { exit }
    ' "$config_file")
