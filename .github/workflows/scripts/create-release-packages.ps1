...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        # Extract command block from config file
        $commandBlock = ""
        if ($configContent -match "(?ms)^${name}:(.*?)(?=^[a-zA-Z]|\Z)") {
            $commandBlock = $matches[1]
        }

        # Extract description
        $description = ""
        if ($commandBlock -match "(?m)^\s*description:\s*(.+)$") {
        ...
        $scriptCommand = ""
        $scriptsBlock = ""
        if ($commandBlock -match "(?ms)^\s*scripts:(.*?)(?=^\s{0,2}[a-zA-Z]|\Z)") {
            $scriptsBlock = $matches[1]
        }

        if ($scriptsBlock -match "(?m)^\s*${ScriptVariant}:\s*(.+)$") {
            $scriptCommand = $matches[1].Trim()
        }

        if ([string]::IsNullOrEmpty($scriptCommand)) {
            Write-Warning "No script command found for $ScriptVariant in $($template.Name) (config: $name)"
            $scriptCommand = "(Missing script command for $ScriptVariant)"
        }

        # Extract agent_script command
        $agentScriptCommand = ""
        $agentScriptsBlock = ""
        if ($commandBlock -match "(?ms)^\s*agent_scripts:(.*?)(?=^\s{0,2}[a-zA-Z]|\Z)") {
            $agentScriptsBlock = $matches[1]
        }

        if ($agentScriptsBlock -match "(?m)^\s*${ScriptVariant}:\s*(.+)$") {
