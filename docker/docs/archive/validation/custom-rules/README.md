...
This directory contains custom YAML validation rules for the Red Hat Migration Toolkit for Applications (MTA) 7.2. These rules enforce the YAML best practices defined in `.config/copilot/yaml-best-practices.yml` across the entire repository.
...
├── file-organization-validation.yaml # File structure rules (9 rules)
└── README.md                         # This file
```
...
mta-cli analyze \
  --input . \
  --output ./output/ \
  --rules ./custom-rules/metadata-validation.yaml

# Indentation rules only
mta-cli analyze \
  --input . \
  --output ./output/ \
  --rules ./custom-rules/indentation-validation.yaml
```
...
```bash
# Windows
start ./output/static-report/index.html
